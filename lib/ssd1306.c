...
void ssd1306_rotated_rect_angle(ssd1306_t *ssd, int cx, int cy, int w, int h, double angle_deg, bool value)
{
    ...
    // Calcula seno e cosseno uma única vez
    float cos_a = cos(angle);
    float sin_a = sin(angle);

    // Pontos do retângulo antes da rotação (centrado em (0,0))
    int x0 = -w / 2, y0 = -h / 2;
    int x1 = w / 2, y1 = -h / 2;
    int x2 = w / 2, y2 = h / 2;
    int x3 = -w / 2, y3 = h / 2;

    // Aplica rotação e translação para cada ponto
    int rx0 = cx + (int)(x0 * cos_a - y0 * sin_a);
    int ry0 = cy + (int)(x0 * sin_a + y0 * cos_a);

    int rx1 = cx + (int)(x1 * cos_a - y1 * sin_a);
    int ry1 = cy + (int)(x1 * sin_a + y1 * cos_a);

    int rx2 = cx + (int)(x2 * cos_a - y2 * sin_a);
    int ry2 = cy + (int)(x2 * sin_a + y2 * cos_a);

    int rx3 = cx + (int)(x3 * cos_a - y3 * sin_a);
    int ry3 = cy + (int)(x3 * sin_a + y3 * cos_a);

    // Desenha as 4 arestas
    ssd1306_line(ssd, rx0, ry0, rx1, ry1, value);
    ssd1306_line(ssd, rx1, ry1, rx2, ry2, value);
    ssd1306_line(ssd, rx2, ry2, rx3, ry3, value);
    ssd1306_line(ssd, rx3, ry3, rx0, ry0, value);
}
